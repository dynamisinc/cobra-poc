...
public class ChecklistServiceTests : IDisposable
{
    ...
    public async Task ArchiveChecklistAsync_SetsIsArchivedToTrue()
    {
        ...
        // Act - Use admin user who has Manage role and can archive any checklist
        var result = await _service.ArchiveChecklistAsync(checklistId, _adminUser);

        // Assert
        Assert.True(result);
        var checklist = await _context.ChecklistInstances.FindAsync(checklistId);
        Assert.True(checklist!.IsArchived);
        Assert.Equal(_adminUser.Email, checklist.ArchivedBy);
        Assert.NotNull(checklist.ArchivedAt);
    }

    [Fact]
    public async Task ArchiveChecklistAsync_ReturnsNull_WhenChecklistNotFound()
    {
        // Act
        var result = await _service.ArchiveChecklistAsync(Guid.NewGuid(), _testUser);

        // Assert - Returns null when checklist not found
        Assert.Null(result);
    }

    [Fact]
    public async Task ArchiveChecklistAsync_ReturnsFalse_WhenContributorNotOwner()
    {
        // Arrange - Seed data creates checklists with different CreatedBy
        await SeedTestData();
        var checklistId = _context.ChecklistInstances.First(c => !c.IsArchived).Id;

        // Act - Contributor cannot archive checklist they didn't create
        var result = await _service.ArchiveChecklistAsync(checklistId, _testUser);

        // Assert - Returns false when contributor is not the owner
        Assert.False(result);
    }
